...
export const checkServerHealth = async (
  ...
  // Set defaults for retries and interval
  ...
  while (count < maxRetries) {
    ...
    try {
      ...
      if (resp.ok && resp.status === 200) {
        ...
        console.warn(
          `Server health check failed (attempt ${count + 1}/${maxRetries}): status ${resp.status}`,
        ); // Log warning with attempt number
      }
    } catch (error) {
      console.error(
        `Server health check error (attempt ${count + 1}/${maxRetries}):`,
        error,
