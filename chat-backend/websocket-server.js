...
const { joinRoom, createRoom, leaveRoom, broadcastToRoom } = require('./room');
...
function startWebSocketServer({ server }) {
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
      ...
      if (data.type === 'join') {
        ...
        if (rooms[roomId]) {
          const messageData = { type: 'message', data: data.message, user: data.user }; // User data can be sent here
  ...
  console.log(`WebSocket server listening on port express server`);
}
